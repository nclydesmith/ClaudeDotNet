...
public class BashTool : ITool
{
    ...
    public string Description => "Execute shell commands in /bin/bash (macOS/Linux).";
    ...
    public async Task<ToolResult> ExecuteAsync(ToolExecutionContext context, CancellationToken cancellationToken)
    {
        ...
        string shell = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? "powershell.exe" : "/bin/bash";
        string args = RuntimeInformation.IsOSPlatform(OSPlatform.Windows) ? $"-Command \"{input.Command.Replace("\"", "\\\"")}\"" : $"-c \"{input.Command.Replace("\"", "\\\"")}\"";
